            ...
            do {
                ...
                for try await progressUpdate in stream {
                    ...
                    case .complete(let summary):
                        // Map V2 summary to V1 UI model to avoid rewriting the result view.
                        let books = [GeminiCSVImportJob.ParsedBook(title: "Created \(summary.booksCreated) books", author: "", isbn: nil, coverUrl: nil, publisher: nil, publicationYear: nil, enrichmentError: nil)]
    ...
    private func fallbackPollingV2(jobId: String) async {
        ...
        while pollCount < maxPolls && !Task.isCancelled {
            do {
                ...
                case "complete":
                    // A more robust implementation would fetch the full results here.
                    // For now, we'll just indicate completion.
                    importStatus = .completed(books: [], errors: [])
                    return
                case "failed":
